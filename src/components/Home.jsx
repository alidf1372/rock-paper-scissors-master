...
const Home = ({ handleScoreChange }) => {
    ...
    useEffect(() => {
        ...
        if (savedScore) {
            ...
            handleScoreChange(parseInt(savedScore));
        }

        setSystemChoice(listOfModes[Math.floor(Math.random() * 3)])
    }, [handleScoreChange]);

    const listOfModes = ['paper', 'rock', 'scissor'];
    ...
    const handlerChosen = (chosenName) => {

        if (chosenName === systemChoice) {
            console.log('DRAW');
        } else if (map[chosenName] === systemChoice) {
            const newScore = score + 1;
            setScore(newScore);
            handleScoreChange(newScore);
            console.log(systemChoice + ' you win');
        } else {
            if (score > 0) {
                const newScore = score - 1;
                setScore(newScore);
            ...
            console.log(systemChoice + ' you loss');
        }
    };

    return (
        <>
            <section className="game_environment">
                <Link to={`/paper/${systemChoice}`}>
                    <div className="icon paper_icon" onClick={() => handlerChosen('paper')}>
                ...
                <Link to={`/scissor/${systemChoice}`}>
                    <div className="icon scissor_icon" onClick={() => handlerChosen('scissor')}>
                ...
                <Link to={`/rock/${systemChoice}`}>
                    <div className="icon rock_icon" onClick={() => handlerChosen('rock')}>
